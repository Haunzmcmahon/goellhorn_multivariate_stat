...
```
```{r}
summary(data$xbstaat)
```

```{r}
# Bar plot of counts for 'xbstaat' by 'werr'
ggplot(data, aes(x = xbstaat, fill = werr)) + 
  geom_bar(position = "dodge") +
  labs(title = "Distribution of Country of Origin by Construction Period ('werr')",
       x = "Country of Origin ('xbstaat')",
       y = "Count") +
  theme_minimal() +
...
```{r fig.width=11.7, fig.height=8.3/2, out.width="100%", dpi=300}
# Bar plot of counts for 'xbgeblan' by 'werr'
ggplot(data, aes(x = werr, fill = xbgeblan )) + 
  geom_bar(position = "dodge") +
  labs(title = "Distribution of Ethnic/National Background by Construction Period ('werr')",
       x = "Ethnic/National Background ('xbgeblan')",
       y = "Count") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for readability
...
```
```{r}
summary(data$xbgeblan)
```
```{r}
summary(data$xhatlevel)
```

```{r}
# Bar plot for xhatlevel (Education Level)
ggplot(data, aes(x = xhatlevel)) +
  geom_bar(fill = "steelblue") +
  labs(title = "Distribution of Education Level",
       x = "Education Level (xhatlevel)",
       y = "Count") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for readability
